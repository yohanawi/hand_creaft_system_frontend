...
export default function WishlistTrustSignals({ isCompact }: Props) {
    return (
        <View className="overflow-hidden rounded-[30px] border border-[#EADBCB] bg-white p-6 mb-32" style={WISHLIST_PANEL_SHADOW}>
            ...
            <View className={`gap-4 ${isCompact ? '' : 'md:flex-row'}`}>
                {TRUST_SIGNALS.map((signal) => (
                    <View key={signal.title} className="flex-1 rounded-[24px] border border-[#F0E3D7] bg-[#FFF8F1] p-4 items-center">
                        ...
                        <Text className="mb-2 text-[19px] leading-6 text-[#271C18] font-body font-semibold">
                            {signal.title}
                        </Text>
                        <Text className="text-[14px] leading-6 text-[#7A685B] text-center" style={{ fontFamily: WISHLIST_SANS }}>
                            {signal.description}
                        </Text>
                    </View>
                ))}
