...
export default function ContactSidebar({
    ...
    return (
        ...
        >
            <View className="rounded-[34px] border px-5 py-6 mb-32" style={{ borderColor: '#ECD9CA', backgroundColor: '#FFF4EA' }}>
                ...
                {/* Studio hours */}
                <View className="mt-5 rounded-[28px] border px-5 py-5" style={{ borderColor: '#E8D7C8', backgroundColor: '#FFFFFF' }}>
                    <View className="flex-row items-center">
                        <Feather name="clock" size={18} color={BROWN.DarkColor} />
                        <Text className="ml-3 font-heading text-[24px]" style={{ color: BROWN.TextPrimary }}>
                            Studio hours
                        </Text>
                    </View>
                    ...
                    <View className="gap-3 mt-4">
                        {STUDIO_HOURS.map((row) => (
                            <View key={row.day} className="flex-row items-center justify-between rounded-[20px] px-4 py-3" style={{ backgroundColor: '#FAF0E8' }}>
                                <Text className="font-body text-[13px]" style={{ color: BROWN.TextSecondary }}>
                                    {row.day}
                                </Text>
                ...
                {/* FAQs */}
                <View className="mt-5 rounded-[28px] border px-5 py-5" style={{ borderColor: '#E8D7C8', backgroundColor: '#FFFFFF' }}>
                    <Text className="font-heading text-[24px]" style={{ color: BROWN.TextPrimary }}>
                        Quick answers
                    </Text>
                    ...
                    <View className="gap-4 mt-4">
                        {FAQS.map((item, index) => {
                            ...
                            return (
                                <View key={index} className="rounded-[22px] px-4 py-4" style={{ backgroundColor: "#FAF5EF" }} >
                                    <TouchableOpacity onPress={() => toggleItem(index)} className="flex-row items-center justify-between">
                                        <Text className="font-body text-[13px] font-semibold leading-6" style={{ color: BROWN.TextPrimary }}>
                                            {item.question}
                                        </Text>
                                        <Text className="ml-2 text-[16px] font-bold" style={{ color: BROWN.TextPrimary }}>
                                            {isOpen ? "−" : "+"}
                                        </Text>
                                    </TouchableOpacity>

                                    {isOpen && (
                                        <Text className="mt-2 font-body text-[13px] leading-6" style={{ color: BROWN.TextSecondary }}>
                                            {item.answer}
                                        </Text>
                                    )}
                                </View>
